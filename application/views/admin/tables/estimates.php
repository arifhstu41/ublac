...
if (@get_module_info('branches')->active == 1) { 
    if(is_admin()){
    ...
    $user_branches = array_column($branches, 'id');

    if(isset($user_branches[0])){

        array_push($where, 'AND '.db_prefix() . 'estimates.branches_id IN (' . implode(', ', $user_branches) . ')');
    }else{
        
        array_push($where, 'AND '. db_prefix() . 'estimates.branches_id < 1');
    }

    $branchesMap = tableMap('branches', 'branch');
}
...
$result = data_tables_init($aColumns, $sIndexColumn, $sTable, $join, $where, [
    ...
    'hash',
    db_prefix() . 'estimates.branches_id',
]);
...
foreach ($rResult as $aRow) {
    ...
    if (@get_module_info('branches')->active == 1) { 
        $row[] = $branchesMap[$aRow['branches_id']];
    }

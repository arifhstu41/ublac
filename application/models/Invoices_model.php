...
class Invoices_model extends App_Model
{
    ...
    {
        unset($data['quantity2']);
        unset($data['quantity_unit']);

        if (isset($data['branches_id']) && $data['branches_id'] > 0) {
            $data['prefix'] = getPrefix($data['branches_id']);
        } else {
    ...
    protected function save_items($items, $id)
    {
        ...
        foreach ($items as $item) {
            ...
            if (update_sales_item_post($item['itemid'], $item, 'quantity')) {
                $updated = true;
            }
            if (update_sales_item_post($item['itemid'], $item, 'quantity_unit')) {
                $updated = true;
            }

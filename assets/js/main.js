...
function add_item_to_table(data, itemid, merge_invoice, bill_expense) {
  ...
  get_taxes_dropdown_template(tax_name, data.taxname).done(function (
    ...
    table_row += '<td><input type="number" min="0" onblur="calculate_total();" onchange="calculate_total();" data-quantity name="newitems[' + item_key + '][quantity]" value="' + data.quantity + '" class="form-control">';
        if (!data.quantity_unit || typeof (data.quantity_unit) == 'undefined') {
            data.quantity_unit = '';
        }

        table_row += '<input type="text" placeholder="' + app.lang.unit + '" name="newitems[' + item_key + '][quantity_unit]" class="form-control input-transparent text-right" value="' + data.quantity_unit + '">';
...
function get_item_preview_values() {
  ...
  response.quantity = $('.main input[name="quantity2"]').val();
  response.quantity_unit = $('.main input[name="quantity_unit"]').val();
  return response;
}
